...
func TestProcessOrderPaymentErrorWhenOrderGatewayRiseErr(t *testing.T) {
    ...
    orderGatewayMock.EXPECT().GetByUuid(ctx, &userUuid, &orderUuid).Return(nil, errOrderGateway)
    ...
    err := processOrderPayment.Execute(ctx, &userUuid, &orderUuid, &paymentOrder)
...
func TestProcessOrderPaymentErrorWhenOrderStatusDiffCreated(t *testing.T) {
    ...
    order := domain.Order{Id: 1, Uuid: orderUuid, UserUuid: userUuid, Number: 1023, Status: domain.PROCESSING,
...
func TestProcessOrderPaymentPaidWhenErrorOrderGatewayUpdate(t *testing.T) {
    ...
    orderGatewayMock.EXPECT().GetByUuid(gomock.Any(), &userUuid, &orderUuid).Return(&order, nil)
